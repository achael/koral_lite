...
#define OUTCOORDS2 OUTCOORDS
  ...
  #ifdef RADIATION
  ldouble erad_arr[NX][NY][NZ];
  ldouble F1_arr[NX][NY][NZ];
  ldouble F2_arr[NX][NY][NZ];
  ldouble F3_arr[NX][NY][NZ];
  
  #ifdef EVOLVEPHOTONNUMBER
  ldouble nphot_arr[NX][NY][NZ];
  #endif
  #endif
...
#pragma omp parallel for private(iz,iy,ix,iv,i)
  for(iz=0;iz<NZ;iz++)
  {
	...
	#ifdef RADIATION
	erad_arr[ix][iy][iz] = erad;
	F1_arr[ix][iy][iz] = urf[1];
	F2_arr[ix][iy][iz] = urf[2];
	F3_arr[ix][iy][iz] = urf[3];

	#ifdef EVOLVEPHOTONNUMBER
	nphot_arr[ix][iy][iz] = nphot;
	#endif
	#endif
