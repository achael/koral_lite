...
{  
  ...
  //which files to read
  int no1,no2,nostep,ifphiavg,ifavg;
  if(argc!=4 && argc!=6)
  {
  ...
  else
  {
    ...
    no2=atof(argv[2]);  //the last file
    nostep=atof(argv[3]); //step through files
    if(argc==6)
    {
      ifavg=atoi(argv[4]); //run on avg files or res files
      ifphiavg=atoi(argv[5]); //run on phi-averaged files or not
    }
    else
    {
      ifavg=1;
  ...
  if (ifphiavg==1 && NZ!=1)
  {
    printf("ifphiavg=1 -- make sure code is compiled with NZ=1!\n");
    exit(-1);
  }
  ...
  if(ifphiavg==0)
    ...
    my_err("using phisli(ced) data with ./avg_dumps makes no sense.\n");
  ...
  printf("working on files %s%04d to %s%04d with %d step \n",base,no1,base,no2,nostep);    
  ...
  for(ifile=no1; ifile<=no2; ifile+=nostep)
  {
	  ...
	  printf("Cannot RESCALEDENSITYPOSTPROC on avg file input!\n");
          printf("-- must run on res files (ifavg=0)\n");
  ...
  //make average of average files
  avgtime=ttot;
  if(ifavg)
    sprintf(bufor,"avgavg%04d-",no1);
  else
    //ANDREW should we denote if it's been rescaled or not??
    sprintf(bufor,"avgres%04d-",no1);
  
  nfout2=no2;
  fprint_avgfile(global_time,folderin,bufor);
