...
int init_dsandvels_limotorus(FTYPE r, FTYPE th, FTYPE a, FTYPE *rhoout, FTYPE *uuout, FTYPE *ell)
{
   ...
   if (eps < 0)
     ...
     rho = pow((-1 + LT_GAMMA)*eps*pow(kappa,-1),pow(-1 + LT_GAMMA,-1));

   uu = kappa * pow(rho, LT_GAMMA) / (LT_GAMMA - 1.);
#endif
...
// Calculate approximate density maximum for limotorus
// Using penna et al eq 24
FTYPE rhomax_limotorus_approx(FTYPE a)
{
  ldouble rho=-1.;
#ifdef LIMOTORUS
  ldouble ell1=lK(LT_R1, a); // angular momentum at transition point
  ldouble rmax1 = LT_XI*LT_XI*ell1*ell1 - 4.; // location of pressure/density maximum
  ldouble rmax;
  if(rmax1 < LT_RIN)
    rmax = LT_RIN;
  else
  ...
  ldouble uint,ell;
  init_dsandvels_limotorus(rmax, 0.5*M_PI, a, &rho, &uint, &ell); // density at the maximum
  //printf("rmax %.4f %.4f %.2e \n", rmax1, rmax, rho);
#endif
  return rho;
}
...
int init_dsandvels_fishbone_moncrief(FTYPE r, FTYPE th, FTYPE a, FTYPE *rhoout, FTYPE *uuout, FTYPE *ellout)
{
  ...
  ell = (rmax*rmax*rmax*rmax + rmax*rmax*a*a - 2.*rmax*a*a - a*sqrt(rmax)*(rmax*rmax - a*a)) / (rmax*rmax - 3.*rmax + 2.*a*sqrt(rmax)) / pow(rmax,1.5);
