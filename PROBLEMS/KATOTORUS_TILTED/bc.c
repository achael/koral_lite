...
if(BCtype==YBCLO) //upper spin axis 
  {      
	...
	iix=ix;
        ...
        #ifdef TRANSMITTING_YBC //Brandon - Not called when MPI is active
        iiy=-iy-1;
        iiz=iz+(TNZ/2);
        if(iiz > TNZ-1) iiz-=TNZ;
        #endif
	...
	for(iv=0;iv<NV;iv++)
	  {
	    ...
	    if(iv==VY || iv==B2 || iv==FY0)
            {
              if(ifinit==0) pp[iv]=-get_u(p,iv,iix,iiy,iiz); //flip only if dealing with B NOT vec pot
              else if(ifinit==1) pp[iv]=get_u(p,iv,iix,iiy,iiz); //flip only if dealing with B NOT vec pot
            }
            #ifdef TRANSMITTING_YBC  //Brandon - Not called when MPI is active
            else if(iv==VZ || iv==B3 || iv==FZ0)
	      pp[iv]=-get_u(p,iv,iix,iiy,iiz);
            #endif
	    else
	      pp[iv]=get_u(p,iv,iix,iiy,iiz);
