...
int diskatboundary(ldouble *pp, void *ggg, void *gggBL)
{
  ...
  ldouble fff,xxx,xoff,sss,dMde0,dMdt,massrescale; //variables for dmdt curve

  //In partial TDE, we determine binding energy from time
  TFB_STR = TFB0;
  sss = F_S; // s in mathematica notebook
  xoff = F_XOFF; // k in mathematica notebook
  massrescale = MASSRESCALE;

  ldouble stream_time = global_time + TFB_STR;
  EPSILON = EPSILON0*pow( (stream_time/TFB_STR) , -2./3.);
  xxx = EPSILON/DEPSILON;
  dMde0 = (MSTAR/MASS)/DEPSILON; // M_*/Deps0 in GU
  fff = 0.1*pow(10., sss*fabs(xxx - xoff)); //dMde in units of M_*/Deps0

  dMdt = massrescale*dMde0*fff*pow(2.*M_PI,2./3.)*pow(stream_time,-5./3.)/3.; //in GU
  MDOTEDDIN = dMdt*massGU2CGS(1.)/(2.48e18*MASS*timeGU2CGS(1.)); //convert to Eddington units

  //printf("dMdt(Edd) - %e \n",MDOTEDDIN);

  DISKVR = -1.*sqrt(2.*(1./RMIN_DISK + EPSILON));
  MDOTIN = (MDOTEDDIN*2.48e18*MASS);
  //end of basic TDE parameters
